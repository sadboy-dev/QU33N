--// Sleitnick Net Remote Tester (FULL VERSION)
...
exec.MouseButton1Click:Connect(function()
	...
	for part in string.gmatch(path, "[^/]+") do
		...
		if not current then
			log.Text ..= "[ERROR] Path invalid: "..path.."\n"
			return
		end
	end
	...
	if current:IsA("RemoteEvent") then
		if finalArg ~= nil then
			current:FireServer(finalArg)
			log.Text ..= "[SEND] FireServer("..tostring(finalArg)..") -> "..path.."\n"
		else
			current:FireServer()
	...
	elseif current:IsA("RemoteFunction") then
		local res = finalArg and current:InvokeServer(finalArg) or current:InvokeServer()
